...
if(isset($_GET['status'])){
	$sql_updatestat = "UPDATE images SET status = '$_GET[status]' WHERE id = '$_GET[id]'";
	$run_updatestat = mysqli_query($connect, $sql_updatestat);
}elseif(isset($_GET['del_id'])){
	$sql_delete = "DELETE FROM images WHERE id = '$_GET[del_id]'";
						...
						<div class="row">
							<div class="col-md-11">
								<table class="table table-hover">
									...
									<tbody>
										<?php
										$sql_selnews = "SELECT * FROM images ORDER BY id DESC";
										...
										while($row = mysqli_fetch_assoc($run_selnews)){
											...
											<td>'.$i.'</td>
											<td>'.$row['date'].'</td>
											<td><img src="'.$row['db_path'].'" width="200"></td>
											<td>'.$row['caption'].'</td>
											<td>'.$row['author'].'</td>
											<td>'.$row['status'].'</td>
											<td>'.($row['status'] == 'publish' ? '<a href="view_image.php?status=draft&id='.$row['id'].'" class="btn btn-warning btn-sm navbar-btn">Draft</a>' : '<a href="view_image.php?status=publish&id='.$row['id'].'" class="btn btn-info btn-sm navbar-btn">Publish</a>').'</td>
											<td><a href="view_image.php?del_id='.$row['id'].'" class="btn btn-danger btn-sm navbar-btn">Delete</a></td>
										</tr>' ;
