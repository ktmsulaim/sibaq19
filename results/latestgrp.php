...
<html>
	...
	<body style="background-color:#fff !important;" class="latestgrp">
...
<!---------------------------------------- main content ---------------------------------------------------------->
			<div class="container-fluid">
				<div class="row">
					...
					<?php
						echo '<div class="owl-carousel latestresults2">';
						$sup = '';
						$sql_seleach = "SELECT p_name, p_code, p_category FROM programmes WHERE status = 'publish' AND p_kind = 'group' ORDER BY pb_date DESC LIMIT 36";
						...
						while($row = mysqli_fetch_assoc($run_seleach)){
							...
							echo '<div class="item">
									...
									foreach($p as $ps => $pcode){
										$sel_cand = mysqli_query($connect, "SELECT c.ug, c.position, c.grade, u.user_id, u.place FROM final_group AS c INNER JOIN users AS u ON c.ug = u.username INNER JOIN programmes p ON c.p_code = p.p_code WHERE c.p_code = '$pcode' AND p.status='publish' AND c.position IN(1,2,3) OR  c.p_code = '$pcode' AND p.status='publish' AND c.grade IN('A','B') ORDER BY c.position_point DESC");
										$item = 1;
										while($row1 = mysqli_fetch_assoc($sel_cand)){
											...
											echo '<li class="ritem'.$item.' col-lg-6 clearfix">
												...
												<div class="part1 float-left">
													<div class="imgCircle">
														<img src="../img/'.$row1['user_id'].'.jpg" class="ugImage">
													</div>
												</div>
												<div class="part2 float-left">
												  <p class="pos">'.$row1['position'].' <sup>'.$sup.'</sup><span class="grd">'.$row1['grade'].'</span></p>
												  <h4 class="ug">'.strtoupper(preg_replace('/@.*/', '', $row1['ug'])).', '.$row1['place'].'</h4>
												  </div>
											  </li>';
											$item++;
										}
