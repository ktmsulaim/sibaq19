...
if(isset($_POST['addmark'])){
	...
	if($p_kind == 'individual'){
		...
		$sql_update = "UPDATE candidate_reg SET j1 = $j1, j2 = $j2, j3 = $j3, total = $total  WHERE cand_id = $ids";
		$run_update = mysqli_query($connect, $sql_update);
		}
	
		if($run_update){
			header('Location: markentry.php?success');
		}else{
			header('Location: markentry.php?failed');
		}	
	}else{
		foreach($_POST['gid'] as $gid=>$ids){			
				...
				$sql_update = "UPDATE group_points SET j1 = $j1, j2 = $j2, j3 = $j3, total = $total WHERE grp_id = '$ids'";
				$run_update = mysqli_query($connect, $sql_update);
				}
...
// clear marks
	...
	if(isset($_GET['clear'])){
		$pcode = $_GET['p_code'];
		$sql_clear = "UPDATE candidate_reg SET j1 = 0, j2 = 0, j3 = 0, total = 0, rank = 0, status = '', prepare = 'false' WHERE prog_code = '$pcode'";
		$run_clear = mysqli_query($sql_clear);
		if($run_clear){
			header('Location: markentry.php?cleared');
		}else{
			header('Location: markentry.php?failed');
		}
	}
...
$msg = '';
	if(isset($_GET['empty'])){
		...
		$msg = '<div class="alert alert-danger col-md-4 m-auto small text-center">Mark entry failed!</div>';
	}elseif(isset($_GET['cleared'])){
		$msg = '<div class="alert alert-success col-md-4 m-auto small text-center">Mark cleared successfully!</div>';
	}
?>
...
<html lang="en">
	<head>
		<title>Sibaq '19 | Results > Mark entry</title>
					...
					<div class="col-lg-9 col-md-9">
						...
						<div class="wrapper">
							...
							<div class="cregWrapper pt-3 pb-3">
								<form action="markentry.php" class="form-horizontal" role="form" method="post">
								...
								<table class="table table-bordered" id="markentry">
									<thead>
										<tr><th colspan="8" class="text-center">
											<?php
												if(isset($_POST['find']) || isset($_GET['edit'])){
									...
									<tbody>
										<?php
											if(isset($_POST['find'])){
													$getcand_sql = "SELECT c.*, p.p_kind FROM candidate_reg c INNER JOIN programmes p ON c.prog_code = p.p_code WHERE c.prog_code = '$_POST[pcode]' ORDER BY c.code_letter";
													...
													while($candrow = mysqli_fetch_assoc($run_getcand)){
														...
														if($candrow['p_kind'] == 'individual'){
															...
															<td><input type="hidden" name="j1[]" class="form-control" max="10" value="'.$candrow['j1'].'">'.($candrow['j1'] == '' ? '<input type="number" name="j1[]" class="form-control" max="10">' : $candrow['j1']).'</td>
															<td><input type="hidden" name="j2[]" class="form-control" max="10" value="'.$candrow['j2'].'">'.($candrow['j2'] == '' ? '<input type="number" name="j2[]" class="form-control" max="10">' : $candrow['j2']).'</td>
															<td><input type="hidden" name="j3[]" class="form-control" max="10" value="'.$candrow['j3'].'">'.($candrow['j3'] == '' ? '<input type="number" name="j3[]" class="form-control" max="10">' : $candrow['j3']).'</td>
															</tr>';
															$i++;
													...
													<?php 
														...
														while($candrow = mysqli_fetch_assoc($run_getcand)){
															$percentage = ($candrow['total'] == '' ? '' : ($candrow['total'] * 100) / 30) . ' %';
																...
																<td>'.($candrow['j1'] == '' ? '<input type="number" name="j1[]" class="form-control" required max="10">' : $candrow['j1']).'</td>
																<td>'.($candrow['j2'] == '' ? '<input type="number" name="j2[]" class="form-control" required max="10">' : $candrow['j2']).'</td>
																<td>'.($candrow['j3'] == '' ? '<input type="number" name="j3[]" class="form-control" required max="10">' : $candrow['j3']).'</td>
																</tr>
																</tr>';
																$i++;
										...
										<?php
											if(isset($_GET['edit'])){
													$getcand_sql = "SELECT c.*, p.p_kind FROM candidate_reg c INNER JOIN programmes p ON c.prog_code = p.p_code WHERE c.prog_code = '$_GET[p_code]' ORDER BY c.code_letter";
													$run_getcand = mysqli_query($connect, $getcand_sql);
													$i = 1;
													$numrow = mysqli_num_rows($run_getcand);
													while($candrow = mysqli_fetch_assoc($run_getcand)){
														if($candrow['p_kind'] == 'individual'){
															...
															<td><input type="hidden" value="'.$candrow['cand_id'].'" readonly class="form-control" name="id[]">'.$candrow['candidate_code'].'</td>
															<td><input type="hidden" value="'.$candrow['author'].'" readonly class="form-control" name="ug[]">'. strtoupper(preg_replace('/@.*/', '', $candrow['author'])).'</td>
															<td><input type="number" name="j1[]" class="form-control" max="10" value="'.$candrow['j1'].'" step=".01"></td>
															<td><input type="number" name="j2[]" class="form-control" max="10" value="'.$candrow['j2'].'" step=".01"></td>
															<td><input type="number" name="j3[]" class="form-control" max="10" value="'.$candrow['j3'].'" step=".01"></td>
															</tr>';
															$i++;
														}else{ ?>
														
													<?php
													$sql_grp_prg = "SELECT * FROM group_points WHERE p_code = '$_GET[p_code]' ORDER BY code_letter";
														...
														while($candrow = mysqli_fetch_assoc($run_getcand)){
																...
																<td>'.$candrow['code_letter'].'</td>
																<td></td>
																<td><input type="hidden" value="'.$candrow['grp_id'].'" readonly class="form-control" name="gid[]"></td>
										...
										<div class="form-group clearfix">
											<?php
												if(isset($_POST['find'])){
													echo '<a href="markentry.php?edit&p_code='.$p_code.'" class="btn btn-secondary input float-right ml-2">Edit</a>';
													echo '<a href="markentry.php?clear&p_code='.$p_code.'" class="btn btn-warning input float-right ml-2">Clear mark</a>';
												}
											?>
