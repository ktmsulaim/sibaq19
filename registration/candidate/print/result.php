...
// programme title, code, category
	...
	$pdf->Cell(63,10,'Selected candidates',0,0,'C');
...
// Title common including chest,ug,name
	...
	$pdf->Cell(73,10,'Candidate Name',1,0,'C');
	$pdf->Cell(28,10,'Chest No',1,0,'C');
	$pdf->Cell(73,10,'Institution',1,1,'C');

// if individual print candidates
if($pkind == 'individual'){
	$sql_selprog = "SELECT c.*,p.p_name,u.place FROM candidate_reg c INNER JOIN programmes p ON c.prog_code = p.p_code INNER JOIN users AS u ON c.author = u.username WHERE c.prog_code = '$pcode' AND c.status = 'Selected' ORDER BY c.code_letter";
	...
	while($row = mysqli_fetch_assoc($run_selprog)){
		...
		$pdf->Cell(73,10,$row['candidate_name'],1,0);
		$pdf->Cell(28,10,$row['candidate_code'],1,0);
		$pdf->Cell(73,10,$ug,1,1);

		$i++;
	}
}else{
	$sql_selprog = "SELECT DISTINCT GROUP_CONCAT(c.candidate_name)AS candidates, GROUP_CONCAT(c.candidate_code)AS chest, g.code_letter, c.author,u.place FROM candidate_reg AS c RIGHT JOIN group_points AS g ON c.author = g.ug INNER JOIN users AS u ON c.author = u.username WHERE g.status = 'Selected' AND c.prog_code = g.p_code AND g.p_code = '$pcode' GROUP BY g.ug ORDER BY g.code_letter ASC";
	$run_selprog = mysqli_query($connect, $sql_selprog);
	$i = 1;
	$pdf->SetFont('Arial','',9);
	...
	while($row = mysqli_fetch_assoc($run_selprog)){
		...
		$font_size = 7;
		$decrement_step = 0.1;
		$line_width = 91; // Line width (approx) in mm
