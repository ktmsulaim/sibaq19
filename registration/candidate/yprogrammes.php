					...
					<div class="col-lg-9 col-md-9">
						...
						<div class="wrapper">
							...
							<div class="cregWrapper pt-3 pb-3">
								...
								<form action="print/programmes.php" method="get" role="form">
									<table class="table table-striped">
												...
												if(isset($_POST['find'])){
													...
													while($candrow = mysqli_fetch_assoc($run_getcand)){ 
														...
														if($pkind == 'individual'){
															echo '<tr>
															<td>'.$i.'</td>
															<td><input type="hidden" name="pcode" value="'.$candrow['prog_code'].'">'.$candrow['code_letter'].'</td>
															<td>'.$candrow['candidate_name'].'</td>
															<td>'.$candrow['candidate_code'].'</td>
															<td>'. strtoupper(preg_replace('/@.*/', '', $candrow['author'])).'</td>
															<td><a id="trash" href="statics.php?del_id='.$candrow['cand_id'].'"><i class="fa fa-trash text-center"></i></a></td>
															</tr>';
															$i++;
														}else{ ?>
														<?php
														 	$getcand_sql = "SELECT * FROM group_points WHERE p_code = '$_POST[pcode]' ORDER BY code_letter";
															$run_getcand = mysqli_query($connect, $getcand_sql);
															$i = 1;
															while($candrow = mysqli_fetch_assoc($run_getcand)){ 
																	echo '<tr>
																	<td>'.$i.'</td>
																	<td><input type="hidden" name="pcode" value="'.$candrow['p_code'].'">'.$candrow['code_letter'].'</td>
										...
										<?php
											if(isset($_POST['find'])){
												if(!empty($_POST['find'])){
													echo '<button id="find" class="btn btn-secondary input" name="find" value="Find" title="Find candidate"><i class="fa fa-print"></i> Print</button>';
												}
											}
										?>
								</form>
